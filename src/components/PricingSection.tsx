import { Check } from "lucide-react";

interface PricingSectionProps {
  onOpenDemo: () => void;
}

const plans = [
  {
    id: "monthly",
    name: "Mensuel",
    subtitle: "Sans engagement",
    price: "XX",
    period: "/utilisateur/mois",
    popular: false,
    features: [
      "Toutes les fonctionnalités Provia BASE",
      "Résiliable à tout moment",
      "Support par email",
      "Mises à jour incluses",
    ],
  },
  {
    id: "yearly",
    name: "Engagement 1 an",
    subtitle: "Économisez 20%",
    price: "XX",
    period: "/utilisateur/mois",
    popular: true,
    features: [
      "Toutes les fonctionnalités Provia BASE",
      "Économisez 20% par rapport au mensuel",
      "Support prioritaire",
      "Mises à jour incluses",
    ],
  },
  {
    id: "biennial",
    name: "Engagement 2 ans",
    subtitle: "Économisez 35%",
    price: "XX",
    period: "/utilisateur/mois",
    popular: false,
    features: [
      "Toutes les fonctionnalités Provia BASE",
      "Économisez 35% par rapport au mensuel",
      "Support VIP dédié",
      "Mises à jour incluses",
    ],
  },
];

export const PricingSection = ({ onOpenDemo }: PricingSectionProps) => {
  return (
    <section id="tarifs" className="relative py-20 lg:py-32 overflow-hidden">
      <div className="glow-teal top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px]" />

      <div className="container mx-auto px-4 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <h2 className="text-3xl lg:text-4xl font-bold mb-4">
            Un seul Provia BASE,{" "}
            <span className="text-gradient-orange">trois options</span> d'engagement
          </h2>
          <p className="text-muted-foreground max-w-2xl mx-auto">
            Choisissez la durée qui correspond à votre vision. Le prix est toujours par utilisateur.
          </p>
        </div>

        <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
          {plans.map((plan) => (
            <div
              key={plan.id}
              className={`glass-card-hover p-6 lg:p-8 relative ${
                plan.popular ? "border-primary/50 scale-105 z-10" : ""
              }`}
            >
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="badge-popular">Le plus populaire</span>
                </div>
              )}

              <div className="text-center mb-6">
                <h3 className="text-xl font-semibold mb-1">{plan.name}</h3>
                <p className="text-sm text-primary">{plan.subtitle}</p>
              </div>

              <div className="text-center mb-6">
                <span className="text-4xl lg:text-5xl font-bold">{plan.price} €</span>
                <span className="text-muted-foreground text-sm">{plan.period}</span>
              </div>

              <ul className="space-y-3 mb-8">
                {plan.features.map((feature, i) => (
                  <li key={i} className="flex items-start gap-3">
                    <div className="w-5 h-5 rounded-full bg-provia-teal/20 flex items-center justify-center mt-0.5">
                      <Check className="w-3 h-3 text-provia-teal" />
                    </div>
                    <span className="text-sm text-muted-foreground">{feature}</span>
                  </li>
                ))}
              </ul>

              <button
                className={`w-full py-3 rounded-xl font-semibold transition-all ${
                  plan.popular
                    ? "btn-primary"
                    : "btn-secondary"
                }`}
              >
                Commencer maintenant
              </button>
            </div>
          ))}
        </div>

        <p className="text-center mt-8 text-muted-foreground">
          Besoin d'en parler ?{" "}
          <button onClick={onOpenDemo} className="text-primary hover:underline">
            Réserver une démo
          </button>
        </p>
      </div>
    </section>
  );
};
